...
test("the cron endpoint rejects untrusted requests and reports configuration or delivery failures", async () => {
  const { crons } = JSON.parse(await source("vercel.json"));
  assert.deepEqual(
    crons.filter(({ path }) => path === "/api/internal/integrations/membership-waitlist/process"),
    [{ path: "/api/internal/integrations/membership-waitlist/process", schedule: "0 13 * * *" }],
  );
