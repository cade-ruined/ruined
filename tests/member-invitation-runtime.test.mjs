...
import { fixture, first, second, newcomer, uuid, person, model, load } from "./helpers/personal-invitation-fixture.mjs";
...
test("input validation bounds bodies and does not accept caller-selected identity or counts", async () => {
  ...
  await assert.rejects(model.readMemberInvitationJson(new Request("https://example.test", { method: "PATCH", headers: { "content-type": "application/json" }, body: "x".repeat(4097) })), { status: 413 });
});
