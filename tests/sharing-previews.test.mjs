...
function productMetadataFixture(overrides = {}) {
  ...
  const route = load("app/store/[handle]/page.tsx", {}, {
    ...
    "@/components/store/ProductPurchase": component,
    "@/components/store/ProductDescription": component,
    "@/data/products": { PRODUCT_TONES: {} },
    "@/lib/shopify": { getProducts: async () => [product] },
  });
  return {
    black, blue,
    metadata: (handle = product.id) => route.generateMetadata({
      params: Promise.resolve({ handle }),
    }),
  };
}

test("the actual member-deployment product route keeps its existing product URL and shares the cassette image", async () => {
  ...
  assert.equal(metadata.openGraph.url, `${origin}/store/sunday-clothes-hoodie`);
...
test("an unknown product clears inherited marketing metadata before the page returns not found", async () => {
  const metadata = await productMetadataFixture().metadata("missing-product");
...
test("public route families share the same Open Graph and Twitter helper", () => {
  const routes = [
    ...
    "app/work/page.tsx", "app/work/[slug]/page.tsx", "app/access/page.tsx", "app/contact/page.tsx",
...
test("private route families use the metadata reset so public marketing cards do not leak into account links", () => {
  for (const route of [
    ...
    "app/my/confirmed/page.tsx", "app/my/join/complete/page.tsx",
  ]) {
