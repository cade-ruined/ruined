...
export async function GET(
  ...
  if (getPlatformConfiguration().mode !== "connected") {
    return NextResponse.json({ error: "Member resources are not connected." }, { status: 503 });
  }
  ...
  if (!resource?.storageBucket || !resource.storagePath) {
    return NextResponse.json({ error: "That resource download is not available." }, { status: 404 });
  }
  const supabase = await createSupabaseServerClient();
  if (!supabase) {
    return NextResponse.json({ error: "Member resource storage is unavailable." }, { status: 503 });
  }
  const { data, error } = await supabase.storage
    .from(resource.storageBucket)
    .createSignedUrl(resource.storagePath, 60, { download: resource.slug });
  if (error || !data.signedUrl) {
    return NextResponse.json({ error: "That resource could not be opened." }, { status: 502 });
  }
