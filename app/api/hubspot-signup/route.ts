...
export async function POST(request: Request) {
  ...
  const pageName = source === "artifacts" ? "Artifacts coming soon" : source === "about" ? "About Ruined" : "Store coming soon";
  try {
    const response = await fetch(`https://api.hsforms.com/submissions/v3/integration/submit/${portalId}/${formId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fields: [{ name: "email", value: email }], context: { pageUri: SITE_URL, pageName } }),
      signal: AbortSignal.timeout(10_000),
    });
    return response.ok ? NextResponse.json({ ok: true }) : NextResponse.json({ error: "HubSpot rejected submission" }, { status: 502 });
  } catch {
    return NextResponse.json({ error: "HubSpot delivery failed" }, { status: 502 });
  }
}
