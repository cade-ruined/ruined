...
function resourceInput(body: ResourceBody | null): OpsAcademyResourceInput {
...
export async function POST(request: Request) {
  ...
  try {
    const resource = await saveOpsAcademyResource(
      access.viewer.authUserId,
      resourceInput(body),
    );
    return opsJson({ resource }, 201);
  } catch (error) {
    if (error instanceof OpsOperatingRepositoryError) return opsRepositoryErrorResponse(error);
    console.error("Academy draft could not be created", {
      errorType: error instanceof Error ? error.name : "UnknownError",
    });
    return opsJson({ error: "The Academy draft could not be created." }, 503);
  }
}
