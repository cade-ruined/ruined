...
export default async function BYOB03RegistrationPage() {
  const events = await getPublicCommunityEvents();
  const event = events.find((candidate) => candidate.id === BYOB_03_EVENT_KEY);
  if (!event) notFound();
  ...
  return (
    <main className="-mt-4 min-h-screen bg-[var(--color-bone)] px-4 pb-20 text-[var(--color-faded)] sm:-mt-6 sm:px-8 sm:pb-24">
      <div className="mx-auto max-w-[86rem]">
        ...
        {event.registration?.status === "Open" && event.status !== "Ended" ? <BYOBRegistrationForm eventKey={BYOB_03_EVENT_KEY} /> : <p className="py-8">Registration is closed for this event.</p>}
      </div>
    </main>
  );
}
