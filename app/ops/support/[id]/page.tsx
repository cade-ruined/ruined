...
export default async function OperationsSupportRequestPage({ params }: { params: Promise<{ id: string }> }) {
  const context = await getSupportPageContext(true);
  const { id } = await params;
  if (context.state === "signed_out") redirect(getSupportAccessUrl(`/ops/support/${id}`));
