...
export function parseMembershipWaitlistInput(value: unknown): MembershipWaitlistSubmission | null {
  ...
  if (input.invitationToken !== undefined && (typeof input.invitationToken !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(input.invitationToken))) return null;
  return { name, emailNormalized, phone: phone || null, ...(typeof input.invitationToken === "string" ? { invitationToken: input.invitationToken } : {}) };
}
