...
import type { PlatformUserLink } from "@/lib/platform/repository";
...
/** Used only after provider verification, inside the invitation's atomic claim. */
export async function claimPublicMembershipSignupInTransaction(
  tx: TransactionSql, viewer: PlatformViewer, plan: MembershipBillingPlan,
  ...
  try {
    ...
    return { authUserId: viewer.authUserId, memberId: member.id, personId };
  } catch (error) {
...
/** Internal provisioning helper; public endpoints require a verified invitation. */
export async function claimPublicMembershipSignup(viewer: PlatformViewer, plan: MembershipBillingPlan): Promise<void> {
  await getApplicationDatabase().begin(tx => claimPublicMembershipSignupInTransaction(tx, viewer, plan));
}
