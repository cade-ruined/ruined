    ...
    <main className="member-journey-page member-circle-page member-profile-dossier mx-auto max-w-[82rem] pb-20 pt-2 sm:pb-24 sm:pt-4" data-member-circle-room>
      {!circle.circle ? (
        <section className="mx-auto flex min-h-[58vh] max-w-4xl flex-col justify-center py-12" data-circle-unassigned>
          <p className="[font-family:var(--font-cadehandy2)] text-[1.65rem] leading-none text-[var(--member-red)] sm:text-[1.9rem]">
            Your Circle
          </p>
          ...
          <p className="mt-7 max-w-xl font-[var(--font-body)] text-base leading-relaxed text-[var(--member-muted)]">
        ...
        <>
          ...
          <div className="mt-14 sm:mt-16">
            <section aria-labelledby="circle-resources-title" className="max-w-4xl">
              <h2 className="ui-heading text-[clamp(1.65rem,3vw,2.45rem)] font-black uppercase leading-[0.88] tracking-[-0.045em] text-[var(--member-ink)]" id="circle-resources-title">
              ...
              {circle.resources.length ? (
                <ol className="mt-5 grid gap-4 sm:grid-cols-2">
                  {circle.resources.map((resource) => (
                    <li key={resource.id}>
                      <Link className="group block rounded-[4px] bg-[var(--member-soft)] p-4 transition-colors hover:bg-[var(--member-soft)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[var(--color-poster)] sm:p-5" href={resource.href}>
                        <p className="[font-family:var(--font-cadehandy2)] text-[1.25rem] leading-none text-[var(--member-red)]">For the room</p>
                        <h3 className="ui-heading mt-2 text-xl font-black uppercase leading-[0.9] tracking-[-0.035em] text-[var(--member-muted)] sm:text-2xl">{resource.label}</h3>
                        {resource.description ? <p className="mt-2 font-[var(--font-body)] text-sm leading-relaxed text-[var(--member-muted)]">{resource.description}</p> : null}
                        <span className="mt-4 inline-block font-[var(--font-body)] text-xs font-bold uppercase tracking-[0.04em] text-[var(--member-muted)] transition-colors group-hover:text-[var(--member-red)]">Open →</span>
                      </Link>
                    </li>
                  ))}
                </ol>
              ) : (
                <p className="mt-4 max-w-md font-[var(--font-body)] text-sm leading-relaxed text-[var(--member-muted)]">
