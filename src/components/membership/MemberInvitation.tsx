...
export default function MemberInvitation({ initialSnapshot, preview = false }: { initialSnapshot: PersonalInvitationSnapshot | null; preview?: boolean }) {
  ...
  const nextExpiry = snapshot?.invitations.map(invitation => invitation.expiresAt)
  ...
  async function createInvitation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/my/invitations", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ recipientName: name, recipientEmail: email, requestId: requestId.current, sendEmail: sendEmail && snapshot.emailReady }) });
      const payload = await response.json();
  ...
  async function updateInvitation(invitation: PersonalMemberInvitation, action: "revoke" | "retry_email") {
    if (!writable || busy || writing.current) return;
    ...
    try {
      ...
      setSnapshot(payload.snapshot); setConfirmCancel(null);
      setStatus(action === "revoke" ? `Invitation for ${invitation.recipientName} cancelled. Its history stays here.` : `Email queued for ${invitation.recipientName}. The original deadline stays the same.`);
          ...
          {visibleInvitations.length > 0 ? <ul className={styles.history}>{visibleInvitations.map(invitation => {
            ...
            return <li key={invitation.id} data-selected={selectedId === invitation.id}>
              ...
              <div className={styles.recordActions}>
                ...
                {(state === "expired" || state === "revoked") ? <a href="#create-invitation" onClick={() => { editRecipient("name", invitation.recipientName); setRecipientEmail(invitation.recipientEmail); }}>Invite again ↗</a> : null}
              </div>
