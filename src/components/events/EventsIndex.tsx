...
export default function EventsIndex() {
  ...
  useEffect(() => {
    const selectFromHash = () => {
      ...
      const [year, month] = event.dateTime
        .slice(0, 7)
        .split("-")
        .map(Number);
  ...
  function selectEvent(eventId: string) {
    ...
    window.history.replaceState(
      null,
      "",
      `${window.location.pathname}${window.location.search}#${eventId}`
    );
  }
    ...
    <section className="bg-[var(--color-bone)] px-4 pb-4 pt-0 text-[var(--color-faded)] sm:px-8 sm:pb-8"><div className="mx-auto grid max-w-[96rem] gap-px bg-black/20 lg:grid-cols-[1.1fr_.9fr]">
      <article className="relative flex min-h-[32rem] flex-col justify-end overflow-hidden bg-[#11100e] p-6 text-white sm:p-10">
        ...
        <div className="relative" aria-live="polite" aria-atomic="true">
          ...
          {selected?.summary && (
            <p className="mt-3 text-xs leading-snug text-white/70 sm:text-sm">
              {selected.summary}
            </p>
          )}
        </div>
      ...
      <div className="bg-[var(--color-bone)] p-5 sm:p-8">
        ...
        <div className="mt-5 grid grid-cols-7 text-center text-xs text-black/40">{WEEKDAYS.map(([short, full]) => <span key={full} aria-label={full}>{short}</span>)}</div>
        <div className="mt-2 grid grid-cols-7 gap-1">{calendarDays.map((day, index) => { if (!day) return <span key={`empty-${index}`} />; const event = eventForDay(day); const active = event?.id === selected?.id; return <button key={day} disabled={!event} onClick={() => event && selectEvent(event.id)} className={`aspect-square border text-sm ${event ? "border-[var(--color-poster)] font-bold" : "border-black/10 text-black/35"} ${active ? "bg-[var(--color-poster)] text-white" : ""}`} aria-label={event ? `Select ${event.title}, ${event.date}` : undefined} aria-pressed={event ? active : undefined}>{day}</button>; })}</div>
