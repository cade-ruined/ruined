...
export default function MemberSupport({ tickets, writable }: { tickets: SupportTicketSummary[]; writable: boolean }) {
  ...
  return (
    <main className="member-journey-page member-support-page mx-auto max-w-[78rem] pb-24 [font-family:var(--font-body)]">
      ...
      <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-12">
        <section aria-labelledby="support-new-title">
          <h2 className="mb-5 text-3xl text-[var(--member-red)]" id="support-new-title"><span className="[font-family:var(--font-cadehandy2)]">New request</span></h2>
          <form onChange={() => { requestKey.current = ""; }} onSubmit={submit}>
            <fieldset className="grid min-w-0 gap-5" disabled={pending}>
              ...
              <p className="-mt-2 text-xs leading-relaxed text-[var(--member-muted)]" id="support-message-safety">Keep passwords, sign-in codes, and card details out of your message.</p>
              {error ? <p className="text-sm leading-relaxed text-[var(--member-red)]" role="alert">{error}</p> : null}
        ...
        <section aria-labelledby="support-requests-title">
          <div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-3xl text-[var(--member-red)]" id="support-requests-title"><span className="[font-family:var(--font-cadehandy2)]">Your requests</span></h2><span className="text-sm text-[var(--member-muted)]">{tickets.length}</span></div>
          <SupportTicketList tickets={tickets} />
          {tickets.length >= 200 ? <p className="mt-3 text-xs text-[var(--member-muted)]">Showing your 200 most recently updated requests. Email us if you need an older request.</p> : null}
        </section>
      </div>
    </main>
  );
