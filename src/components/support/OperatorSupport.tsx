...
export default function OperatorSupport({ tickets, writable, emailReady = false }: { tickets: SupportTicketSummary[]; writable: boolean; emailReady?: boolean }) {
  ...
  const filtered = useMemo(() => tickets.filter((ticket) => {
    if (status === "unresolved" ? ticket.status === "resolved" : status !== "all" && ticket.status !== status) return false;
    ...
    <OperatorPageFrame title="Support">
      <div className="mx-auto max-w-[78rem] [font-family:var(--font-body)]">
        ...
        <div className="my-7 grid items-end gap-5 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <label><span className={SUPPORT_LABEL_CLASS}>Status</span><select className={SUPPORT_FIELD_CLASS} onChange={(event) => setStatus(event.target.value)} value={status}><option value="unresolved">All unresolved</option><option value="all">All statuses</option>{SUPPORT_STATUSES.map((item) => <option key={item.value} value={item.value}>{supportStatusLabel(item.value, true)}</option>)}</select></label>
