...
export default function OperatorArtifactProductPicker({ selected, onSelect, disabled, preview }: {
  ...
  return <fieldset className="min-w-0 sm:col-span-full" disabled={disabled || loading}>
    ...
    {selected ? <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-[4px] bg-[var(--color-shop)]/35 p-3">
      <p className="min-w-0 break-words text-sm"><span className="block text-xs text-black/55">Selected product</span><strong>{selected.title}</strong></p>
      ...
      {message ? <p className={`mt-2 text-sm ${error ? "text-[var(--color-poster)]" : "text-black/60"}`} role={error ? "alert" : "status"}>{message}</p> : <p className="mt-2 text-xs text-black/55">Choose an existing product published to the Ruined storefront. This does not create an order.</p>}
