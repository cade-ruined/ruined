...
export default function OperatorAccessEditor({ entry, circles, preview, onClose, onSaved }: {
  ...
  return <div className="fixed inset-0 z-[160] flex justify-end bg-black/55" onMouseDown={(event) => { if (event.target === event.currentTarget && !pending) onClose(); }}>
    <div aria-labelledby="edit-operator-title" aria-modal="true" className="h-full w-full max-w-xl overflow-y-auto bg-[var(--color-bone)] p-4 text-[var(--color-faded)] shadow-xl sm:p-5" ref={dialog} role="dialog" tabIndex={-1}>
      <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-black/60">{entry.displayName}</p><h2 className="operator-record-title" id="edit-operator-title">Edit access</h2><p className="mt-2 break-all text-sm text-black/60">{entry.email}</p></div><button className="min-h-11 px-2 text-sm underline" disabled={pending} onClick={onClose} type="button">Close</button></div>
