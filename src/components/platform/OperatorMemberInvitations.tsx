...
export default function OperatorMemberInvitations({ data, preview = false, directoryParams = {}, showAdd = true }: {
  data: PendingMemberInvitationPage | null; preview?: boolean; directoryParams?: Record<string, string>; showAdd?: boolean;
}) {
  ...
  return <div data-operator-pending={busy ? "true" : undefined}>
    {showAdd ? <OpsInvitationActions preview={preview} onSaved={() => router.refresh()} /> : null}
    <section aria-labelledby="pending-member-joining-title" className="scroll-mt-32" id="pending-member-joining">
      <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="ui-heading text-base font-semibold" id="pending-member-joining-title">Pending joining</h3><button className="min-h-11 text-sm underline underline-offset-4" disabled={busy} onClick={() => { setReviewed(null); setConfirmation(null); router.refresh(); }} type="button">Refresh list</button></div>
