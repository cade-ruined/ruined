...
export default function OperatorAcademyEditor({ editor, preview = false }: { editor: OpsAcademyEditorData; preview?: boolean }) {
  ...
  return (
    <OperatorPageFrame title={`Academy / ${resource.title}`}>
      ...
      <header className="mt-3 grid gap-5 rounded-[4px] bg-[#080605] p-5 text-[var(--color-bone)] sm:p-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div>
          ...
          <h1 className="mt-3 font-[var(--font-display)] text-3xl leading-tight sm:text-4xl">{resource.title}</h1>
          <p className="mt-3 text-sm text-white/65">Save your edits as a draft, then publish the version members should see.</p>
        </div>
        {editor.canManage ? (
          <OperatorAcademyResourceStateActions
      ...
      <section aria-label="Lesson editor" className="mt-7 rounded-[4px] bg-black/[0.03] p-5 sm:p-7">
        <OperatorAcademyEditorForm options={editor.options} resource={resource} preview={preview} />
      </section>
