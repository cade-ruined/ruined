...
export default function MembershipWaitlistForm({ tone = "dark", invitationToken, disabled = false }: { tone?: "dark" | "paper"; invitationToken?: string; disabled?: boolean } = {}) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (disabled || submitting.current || state === "sent") return;
    ...
    <form aria-label="Membership waitlist" aria-busy={state === "sending"} className={`${styles.form} ${tone === "paper" ? styles.paper : ""}`} onSubmit={submit}>
      {state !== "sent" && (
        <>
          ...
          <fieldset className={styles.fields} disabled={disabled || state === "sending"}>
            ...
            <button className={styles.submit} disabled={disabled || state === "sending"} type="submit">
              {state === "sending" ? invitationToken ? "Sending…" : "Joining…" : invitationToken ? "Request to join" : "Join the waitlist"}
